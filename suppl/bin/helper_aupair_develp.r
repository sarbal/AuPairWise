...
shuffle_updated <- function( X, X.r, s, n.factor, dist="other", mode="post"){
...
predict_sample_updated <- function( X, s,n.factor, n.repeats, shuff, filter, dist="other", mode="post" ){
	
	k =  x[filter[[1]],]
	nK = dim(k)[1]

        X.r    = t(apply (X,1,rank, ties.method="random", na.last="keep") )
        X.new = sapply( 1:n.repeats, function(i) shuffle_updated(X,X.r,shuff[i],n.factor)[,shuff[i]] )
        ...
        for( i in 1:n.repeats){
                X.comb.r =  X
                X.comb.r[,shuff[i]] = X.new[,i]
                X.comb.r  =  t(apply (X.comb.r,1,rank, ties.method="average", na.last="keep") )
                Z.ranks[[i]] = t(sapply( 1:nK, function(j) lm.studentized( X.comb.r[k[j,1],],X.comb.r[k[j,2],]), simplify=T ))
                k2 = t(apply(all_pairs ( cbind(sample(N,nK*2), sample(N,nK*2))), 1, as.numeric))

		Z.ranks.r[[i]] = t(sapply( 1:nK, function(j) lm.studentized( X.comb.r[k2[j,1],],X.comb.r[k2[j,2],]), simplify=T ))
        }
        toc()


	all = lapply(1:n.repeats, function(i) matrix(unlist(Z.ranks[[i]]== nS) , ncol=nS, nrow=nK, byrow=F) )
        all.r = lapply(1:n.repeats, function(i) matrix(unlist(Z.ranks.r[[i]]== nS) , ncol=nS, nrow=nK, byrow=F) )

	scores = lapply(1:n.repeats, function(i) rank(colSums(all[[i]]), ties.method="random") )
        labels = lapply(1:n.repeats, function(i) scores[[i]]*0 )
        for( i in 1:n.repeats){
                labels[[i]][shuff[i]] = 1
        }
...
predict_sample_old <- function( X, s,n.factor, k, nS, nK, filter, dist="other", mode="post" ){
...
predict_sample_rand <- function( X, s,n.factor, nS, nK, dist="other", mode="post" ){

	X.new  = shuffle(X,s, n.factor, dist,mode)
	X.r    = t(apply (X.new,1,rank, ties.method="average", na.last="keep") )
	nX1 = dim(X)[1]
	k = t(apply(all_pairs ( cbind(sample(nX1,nK*2), sample(nX1,nK*2))), 1, as.numeric))
	Z.ranks = t(sapply( 1:nK, function(i) lm.studentized( X.r[k[i,1],],X.r[k[i,2],]), simplify=T ))
...
predict_sample_multi <- function( X, sS,n.factor, k, nS, nK, filter, dist="other", mode="post" ){
	...
	for( s in sS) {
        ...
        X.r    = t(apply (X.save,1,rank, ties.method="average", na.last="keep") )
...
write_out_summary <- function(out, results.all, length, pairs, n.factors, n.repeats, AUROC.default = 0.8, nn=100){
	...
	range = n.factors[-1]
		...
		plot(0, 0, ylim=c(0.4,1), type="l", lwd=3, col="lightgrey", xlab="Noise factor", ylab="AUROC", axes=F)
        	axis(2)
        	axis(1, lab=range, at=log10(range) )
	...
	for ( i in 1:length ){
		...
		predictions = approx( n.factors, data$aurocs[i,], n=100)
	        ...
	        AUROC = AUROC.default
                col.def = which(colnames(stats)==AUROC.default)

                lines(predictions, type="l", lwd=3, col="lightgrey")
        	...
        	abline( v = log10(stats[i,col.def]), lwd=3, lty=3, col=2)
